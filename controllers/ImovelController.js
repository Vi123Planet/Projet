...
class ImovelController {
  static async cadastrar(req, res) {
    ...
    await novoImovel.save();
      ...
      await ImovelModel.findOneAndUpdate({_id: req.body._id},
        {
          id: req.body.id,
          localizaçao: req.body.localizaçao,
          proprietario: req.body.proprietario,
  ...
  static async listar(req, res) {
    ...
    const imovels = await UsuarioModel.find();

    res.render("imovel/relatorio", { imoveis, status });

    // const id = req.params.id;
    // const imovel = await UsuarioModel.findOneAndDelete({id: id});
    // const removido = req.query.r;
    // // res.redirect("/usuarios?r=2")
    // res.render("imovel/relatorio",  { usuarios, salvo, removido })
  }
  ...
  static async cadastrarGet(req, res) {
    ...
    const imovel = {};
    if (id != undefined){
      imovel = await ImovelModel.findOne({_id: _id})
    }
